...
import net.forixaim.battle_arts.initialization.registry.BattleArtsItems;
        ...
        eventListener.registerEvent(EpicFightEventHooks.Entity.DELIVER_DAMAGE_POST, event ->
        {
            ...
            if (event.getDamageSource().getAnimation() == SquireSwordAnimations.SQUIRE_SWORD_HEAVY_BLOW)
            {
                if (event.getEntityPatch().getOriginal().getMainHandItem().is(BattleArtsItems.ERDRICKS_SWORD))
                    event.getTarget().playSound(BattleArtsSounds.CRITICAL_HIT.get(), 1.0f, 1.0f);
                if (event.getEntityPatch().getOriginal().getMainHandItem().is(BattleArtsItems.DRAGOVIAN_KING_SWORD))
                    event.getTarget().playSound(BattleArtsSounds.CRITICAL_HIT_DQ8.get(), 1.0f, 1.0f);
            }
        }, this);;
    }
